...
import { getSmeDocRef, getSmeDocument, messaging, updateQueueForSME } from '@/utils/firebase';
...
const Userface: React.FC = () => {
            ...
            try {
                ...
                if(response) {
                    ...
                    const customerExist = customers.find((customer) => {
                        ...
                        return customer.customerName === storedName && customer.customerEmail === storedEmail;
                    });
                    ...
                    if (customerExist) {
                        ...
                        toast({
                            description: 'Kindly fill in your details to check your position on the queue',
                        })
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(getSmeDocRef(smeEmail as string), (doc) => {
          if (doc.exists()) {
            const data = doc.data();
            setTickets([
              ...data.queue.map((customer: TCustomer) => ({
                ...customer,
    ...
    const handleSubmit = async () => {
        if (name && email && phoneNumber) {
            ...
            try {
                const response = await updateQueueForSME(smeEmail as string, [...tickets, {
            ...
            toast({
                description: 'Please fill in your details. Ensure to fill in all fields',
            });
        }
    };
        ...
        const requestPermission = async () => {
            try {
              ...
              if (permission === 'granted') {
                ...
                const token = await getToken(messaging, { vapidKey: 'BNRQuoKGV3t8p-ahe3u0L2TC5tVbuNPJYg4MpZAabYCOyvYjACKFg-uEtJW1EXUB9q5M27JQSHo-K70QjmPpDM8' });
                if (token) {
                  ...
                  if(customerName && customerEmail && customerPhone) {
                    tickets.map(async (ticket) => {
                        if (ticket.customerName === customerName && ticket.customerEmail === customerEmail && ticket.customerPhone === customerPhone) {
                            ...
                            <tbody>
                                {tickets.map((ticket) => (
                                    <tr key={ticket.ticketNo} className="hover:bg-gray-100">
                                        <td className="border px-4 py-2 text-center">{formatTicketId(ticket.ticketNo)}</td>
                                        <td className="border px-4 py-2 text-center">{ticket.customerName}</td>
                                        <td className="border px-4 py-2 text-center">{ticket.customerEmail}</td>
                                        <td className="border px-4 py-2 text-center">{ticket.customerPhone}</td>
